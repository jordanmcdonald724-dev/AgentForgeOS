"""Agent supervisor coordinating pipeline execution with guards."""

import asyncio
import logging
import os
import time
from pathlib import Path
from typing import Any

from services.agent_pipeline import AGENT_PIPELINE, PipelineContext
from control.ai_router import AIRouter
from control.file_guard import FileGuard
from control.execution_monitor import ExecutionMonitor, execution_monitor
from control.scoring_engine import ScoringEngine
from control.recovery_engine import RecoveryEngine
from control.learning_controller import LearningController
from control.dynamic_pipeline_builder import DynamicPipelineBuilder
from control.agent_factory import AgentFactory
from services import AgentService

logger = logging.getLogger(__name__)

def _env_int(name: str, default: int, *, min_value: int = 0, max_value: int = 10**9) -> int:
    raw = os.getenv(name, "")
    try:
        val = int(raw)
    except Exception:
        val = default
    return max(min_value, min(max_value, val))


def _env_float(name: str, default: float, *, min_value: float = 0.0, max_value: float = 1e9) -> float:
    raw = os.getenv(name, "")
    try:
        val = float(raw)
    except Exception:
        val = default
    return max(min_value, min(max_value, val))


def _dir_size_bytes(root: Path, *, stop_at: int) -> int:
    total = 0
    try:
        for p in root.rglob("*"):
            try:
                if not p.is_file():
                    continue
                total += p.stat().st_size
                if total >= stop_at:
                    return total
            except Exception:
                continue
    except Exception:
        return total
    return total


class AgentSupervisor:
    """Coordinates agent execution order and enforces guard checks.

    Agent classes are loaded from ``services.agent_registry.AGENT_REGISTRY``
    (falling back to ``agents.AGENT_CLASS_MAP`` for backward compatibility).
    The supervisor instantiates each agent in pipeline order and delegates
    execution to the typed agent.  If no class is registered for a role the
    generic ``AgentService.run_agent`` is used instead.

    Each agent execution is wrapped in a try/except so that a single agent
    failure never freezes the pipeline — the error is logged and the pipeline
    continues to the next stage.
    """

    def __init__(
        self,
        agent_service: AgentService,
        router: AIRouter | None = None,
        guard: FileGuard | None = None,
        monitor: ExecutionMonitor | None = None,
        scorer: ScoringEngine | None = None,
        recovery: RecoveryEngine | None = None,
        learning: LearningController | None = None,
        pipeline_builder: DynamicPipelineBuilder | None = None,
        agent_factory: AgentFactory | None = None,
    ):
        self.agent_service = agent_service
        self.router = router or AIRouter()
        self.guard = guard or FileGuard()
        # Default to the shared monitor so UI websocket streaming stays consistent.
        self.monitor = monitor or execution_monitor
        self.scorer = scorer or ScoringEngine()
        self.recovery = recovery or RecoveryEngine()
        self.learning = learning or LearningController()
        self.pipeline_builder = pipeline_builder or DynamicPipelineBuilder()
        self.agent_factory = agent_factory or AgentFactory()
        self._agent_classes = self._load_agent_classes()

    @staticmethod
    def _load_agent_classes() -> dict[str, type[Any]]:
        """Load agent class map from the registry; fall back to AGENT_CLASS_MAP."""
        try:
            from services.agent_registry import AGENT_REGISTRY
            return dict(AGENT_REGISTRY)
        except Exception:
            pass
        try:
            from agents import AGENT_CLASS_MAP  # type: ignore
            return dict(AGENT_CLASS_MAP)
        except Exception:
            return {}

    def pipeline(self) -> list[str]:
        """Return the configured pipeline order."""
        return list(AGENT_PIPELINE)

    def can_modify(self, path: str, task_description: str) -> bool:
        """Check if a task may modify a given path."""
        category = self.router.classify(task_description)
        return self.guard.is_allowed(path, category)

    async def run_pipeline(
        self, user_request: str, context: dict[str, Any] | PipelineContext | None = None
    ) -> list[dict[str, Any]]:
        """Execute the agent pipeline sequentially, passing context forward.

        A :class:`~services.agent_pipeline.PipelineContext` object is created
        (or wraps the supplied dict) and passed to every agent so outputs can
        be shared across stages.

        Each agent is called with whichever method it exposes — ``execute``
        takes priority over ``run``.  Any exception raised by an agent is
        caught, logged, and recorded as a failure response; the pipeline then
        continues to the next stage.
        """
        # Build a PipelineContext so agents can share data across stages.
        pipeline_ctx: PipelineContext
        if isinstance(context, PipelineContext):
            pipeline_ctx = context
        else:
            pipeline_ctx = PipelineContext()
            if isinstance(context, dict):
                for k, v in context.items():
                    pipeline_ctx.set(k, v)

        # Pre-execution learning/context injection.
        self.learning.before_execution(pipeline_ctx)

        max_prompt_chars = _env_int("AGENTFORGE_MAX_PROMPT_CHARS", 50000, min_value=1, max_value=500000)
        max_pipeline_steps = _env_int("AGENTFORGE_MAX_PIPELINE_STEPS", 24, min_value=1, max_value=200)
        max_step_seconds = _env_float("AGENTFORGE_MAX_STEP_SECONDS", 600.0, min_value=1.0, max_value=24 * 3600.0)
        max_pipeline_seconds = _env_float("AGENTFORGE_MAX_PIPELINE_SECONDS", 3600.0, min_value=1.0, max_value=7 * 24 * 3600.0)
        max_workspace_mb = _env_int("AGENTFORGE_MAX_WORKSPACE_MB", 10240, min_value=128, max_value=1024 * 1024)
        max_workspace_bytes = max_workspace_mb * 1024 * 1024

        started_monotonic = time.monotonic()

        responses: list[dict[str, Any]] = []
        scores: dict[str, dict[str, float]] = {}
        prompt = user_request

        if not isinstance(prompt, str) or not prompt.strip():
            return [{"success": False, "data": None, "error": "prompt is required"}]
        if len(prompt) > max_prompt_chars:
            return [{"success": False, "data": None, "error": f"prompt exceeds max length ({max_prompt_chars} chars)"}]

        # Allow dynamic pipeline shaping before execution.
        pipeline_order = self.pipeline_builder.build(self.pipeline(), pipeline_ctx)
        if isinstance(pipeline_order, list) and len(pipeline_order) > max_pipeline_steps:
            pipeline_order = list(pipeline_order)[:max_pipeline_steps]

        ws_raw = pipeline_ctx.get("workspace_path") or os.getenv("WORKSPACE_PATH", "")
        if isinstance(ws_raw, str) and ws_raw.strip():
            try:
                ws_path = Path(ws_raw).expanduser().resolve()
                current_size = _dir_size_bytes(ws_path, stop_at=max_workspace_bytes + 1)
                if current_size > max_workspace_bytes:
                    return [{"success": False, "data": None, "error": f"workspace exceeds max size ({max_workspace_mb}MB)"}]
            except Exception:
                pass
        pipeline_ctx.set(
            "execution_limits",
            {
                "max_prompt_chars": max_prompt_chars,
                "max_pipeline_steps": max_pipeline_steps,
                "max_step_seconds": max_step_seconds,
                "max_pipeline_seconds": max_pipeline_seconds,
                "max_workspace_mb": max_workspace_mb,
            },
        )

        pipeline_id = pipeline_ctx.get("pipeline_id", "unknown")
        task_type_map = {
            "Project Planner": "planning",
            "System Architect": "planning",
            "Task Router": "planning",
            "Module Builder": "coding",
            "API Architect": "coding",
            "Data Architect": "planning",
            "Backend Engineer": "coding",
            "Frontend Engineer": "ui_design",
            "AI Integration Engineer": "coding",
            "Integration Tester": "debugging",
            "Security Auditor": "debugging",
            "System Stabilizer": "debugging",
        }

        for step_index, agent_name in enumerate(pipeline_order):
            if (time.monotonic() - started_monotonic) > max_pipeline_seconds:
                responses.append(
                    {
                        "success": False,
                        "data": None,
                        "error": f"pipeline timeout after {int(max_pipeline_seconds)}s",
                        "pipeline_id": pipeline_id,
                        "step_index": step_index,
                    }
                )
                break
            agent_cls: type[Any] | None = self._agent_classes.get(agent_name)
            pipeline_ctx.set("pipeline_id", pipeline_id)
            pipeline_ctx.set("step_index", step_index)
            pipeline_ctx.set("task_type", task_type_map.get(agent_name, "conversation"))
            pipeline_ctx.set("agent_id", agent_name)
            self.monitor.start_step(agent_name, pipeline_ctx)
            try:
                if agent_cls is not None:
                    # roles in AGENT_PIPELINE are unique; instantiate per step/run to avoid any state leakage between stages.
                    agent = self.agent_factory.create(agent_cls, self.agent_service)
                    # Prefer execute(context) per spec; fall back to run(prompt).
                    if hasattr(agent, "execute"):
                        response = await asyncio.wait_for(agent.execute(pipeline_ctx), timeout=max_step_seconds)
                    else:
                        response = await asyncio.wait_for(agent.run(prompt, context=pipeline_ctx.data), timeout=max_step_seconds)
                else:
                    response = await asyncio.wait_for(
                        self.agent_service.run_agent(
                            prompt, context=pipeline_ctx.data, task_type=pipeline_ctx.get("task_type", "conversation")
                        ),
                        timeout=max_step_seconds,
                    )
            except asyncio.TimeoutError:
                response = {
                    "success": False,
                    "data": None,
                    "error": f"{agent_name}: timeout after {int(max_step_seconds)}s",
                }
            except Exception as exc:
                logger.error("Agent '%s' raised an exception", agent_name, exc_info=True)
                response = {
                    "success": False,
                    "data": None,
                    "error": f"{agent_name}: {exc}",
                }

            if not response.get("success"):
                self.monitor.record_error(agent_name, str(response.get("error")), pipeline_ctx)
                # Attempt recovery once.
                response = self.recovery.recover(
                    agent_name, response, pipeline_ctx, attempt=1
                )

            response["pipeline_id"] = pipeline_id
            response["step_index"] = step_index
            self.monitor.end_step(agent_name, response)
            scores[agent_name] = self.scorer.score(agent_name, response, pipeline_ctx)
            responses.append(response)

            if not response.get("success"):
                logger.warning(
                    "Agent '%s' reported failure; continuing pipeline.", agent_name
                )
                pipeline_ctx.set(f"{agent_name}:error", response.get("error"))
                continue

            # Advance the prompt to the agent's text output for the next stage.
            data = response.get("data")
            if isinstance(data, dict) and "text" in data:
                prompt = data["text"]
                pipeline_ctx.set(agent_name, data["text"])

        # Post-execution learning hook.
        self.learning.after_execution(responses, scores, pipeline_ctx)

        return responses
