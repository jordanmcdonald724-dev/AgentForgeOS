...
class ExecutionMonitor:
    ...
    def __init__(self) -> None:
        ...
        self._pipeline_logger = logging.getLogger("pipeline")
    ...
    def record_error(self, step: str, error: str, context: Optional[Any] = None) -> None:
    ...
    def _record_event(
        ...
        try:
            ...
            try:
                self._pipeline_logger.info(json.dumps({"event": evt.event_type, "pipeline_id": evt.pipeline_id, "step_index": evt.step_index, "agent_name": evt.agent_name, "timestamp": evt.timestamp, "data": evt.data}))
            except Exception:
                pass
        except Exception:
            return
...
def _pipeline_id_from_context(ctx: Any) -> str:
    if PipelineContext is not None and isinstance(ctx, PipelineContext):
        return _pipeline_id_from_context(getattr(ctx, "data", {}))
...
def _step_index_from_context(ctx: Any) -> Optional[int]:
    if PipelineContext is not None and isinstance(ctx, PipelineContext):
        return _step_index_from_context(getattr(ctx, "data", {}))
    if isinstance(ctx, dict):
...
def _context_to_data(ctx: Any) -> Dict[str, object]:
    if PipelineContext is not None and isinstance(ctx, PipelineContext):
        return _context_to_data(getattr(ctx, "data", {}))
    if isinstance(ctx, dict):
        return dict(ctx)
    return {}
