...
* **Control layer coverage:** `control/` currently contains `ai_router.py`, `agent_supervisor.py`, `file_guard.py`, and `module_registry.py`. The monitoring, scoring, recovery, dynamic pipeline, and learning controllers called out in this spec (`execution_monitor.py`, `scoring_engine.py`, `recovery_engine.py`, `learning_controller.py`, `dynamic_pipeline_builder.py`, `agent_factory.py`) are not yet present and should be prioritized to satisfy the control spine and enforcement rules.
* **Pipeline + agents:** A sequential pipeline and shared context are defined in `services/agent_pipeline.py`, with supervisor dispatch in `control/agent_supervisor.py` and agent implementations under `agents/` (all inheriting `agents/base_agent.py`). No monitoring, scoring, or recovery hooks are wired into the pipeline yet.
* **Memory + knowledge:** Memory utilities (`services/mongo_memory.py`, `services/memory_manager.py`) and knowledge components (`knowledge/embedding_service.py`, `knowledge/vector_store.py`, `knowledge/knowledge_graph.py`) exist but are not currently orchestrated by a `learning_controller` at runtime.
...
* **Next-start focus:** Implement the missing control-layer monitors (execution monitor, scoring engine, recovery engine) and the learning controller to enforce the “every step monitored/scored/recoverable” rule, then introduce dynamic pipeline building and the specialized Builds/Assets/Sandbox pipelines to match the universal pipeline backbone.
